...
| `user_id` | `uuid` | はい | このテストの所有者 | `8f3...` | Supabase Auth の `auth.users.id` を保持する。アプリ独自の `users` テーブルは MVP で持たない |
...
| `exams.user_id` | Supabase Auth のユーザー削除に伴うアプリ側クリーンアップは server API で行う（`auth.users` への FK/CASCADE に依存しない） |
...
- **ID 方針**:
  - MVP では `exams`, `exam_subjects`, `study_plans`, `daily_plans`, `progress_logs`, `exam_results`, `availability_rules` の全テーブルで UUID をクライアント生成する
  - この方針はゲスト保存時とログイン後保存時で統一する
  - ゲスト移行時は client-generated UUID をそのまま持ち込める形にする
- **移行処理の実装方針**:
  ...
  - 再送・通信断に備えて冪等性を保証する
  - 同一 import の完全再送については `INSERT ... ON CONFLICT DO NOTHING` を使って無害化する
  - 既存 ID が他ユーザー所有物と衝突した場合は import を拒否し、`409 Conflict` として扱う
